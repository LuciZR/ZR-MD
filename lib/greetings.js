...
async function send(m, conn, text) {
        if(m.id == "120363318748857871@g.us" && conn.user.number != "918293838182") return;
        let {
                id,
                subject,
                desc,
        ...
        if (URL && URL.endsWith('.mp4')) {
                ...
                try {
                        ...
                        msg.image = {url:"https://static.zerochan.net/Aizen.Sousuke.full.3013838.jpg"};
                }
...
module.exports = async (m, conn, {welcome, exit}) => {
        ...
        try {
                if (m.action == 'add') {
                        if (!welcome || welcome.status == 'true' || !welcome.message) return;
